...
def main():
    with torch.cuda.device(DEVICE_IDS[0]):
        p_model = _model_init()
        #----data loader----
        train_loader = get_dataset(files_vec=['pos_12.txt', 'neg_12.txt', 'part_12.txt', 'angle_12_aug.txt'])
        ...
        for i in range(MAX_ITERS):
            ...
            _angle_labels = _angle_labes.cuda(DEVICE_IDS[0])
            _angle_labels_var = Variable(_angle_labels)

            outputs = p_model(_images) #----model forward 
            loss_cls, loss_bbox, loss_angle = p_model.get_loss(outputs, _bbox, _labels_var)
